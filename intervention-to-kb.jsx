...
export default function InterventionToKB() {
...
Convertis ces données en entrée structurée pour la base de connaissances.`;
    ...
    try {
      ...
      // Inject GLPI ticket id if provided
  ...
  const containerStyle = {
    fontFamily: font,
    background: `linear-gradient(135deg, ${colors.bg} 0%, #0f172a 50%, #0a0e1a 100%)`,
    minHeight: "100vh",
    color: colors.text,
    padding: "0",
  };
        ...
        <div style={{ maxWidth: "960px", margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "8px" }}>
            <div style={{
              ...
              fontSize: "20px", boxShadow: `0 4px 16px rgba(59,130,246,0.3)`,
            }}>
              ⚡
            </div>
            <div>
              <h1 style={{ margin: 0, fontSize: "22px", fontWeight: 700, color: colors.white, letterSpacing: "-0.01em" }}>
                Intervention → Base de Connaissances
              </h1>
              <p style={{ margin: 0, fontSize: "13px", color: colors.textMuted }}>
                Composant 1 — Convertisseur IA de fiches d'intervention en entrées KB
              </p>
            </div>
          </div>
        </div>
      ...
      <div style={mainStyle}>
        ...
        {currentStep === "form" && (
          <div style={{ animation: "fadeIn 0.4s ease" }}>
            ...
            {error && (
              <div style={{
                background: "rgba(239,68,68,0.1)", border: `1px solid ${colors.danger}44`,
                borderRadius: "8px", padding: "12px 16px", marginBottom: "20px",
                color: colors.danger, fontSize: "13px",
              }}>
                ⚠️ {error}
              </div>
            )}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                <div>
                  ...
                  <select
                    ...
                    style={{ ...inputStyle, cursor: "pointer" }} {...selectFocusHandlers}
                  >
                    <option value="">— Sélectionner —</option>
                    {CATEGORIES.map((c) => (
                      <option key={c.id} value={c.id}>{`${String(c.id).padStart(2, "0")} - ${c.name}`}</option>
                    ))}
                  </select>
                </div>
                <div>
                  ...
                  <select
                    ...
                    style={{ ...inputStyle, cursor: "pointer", opacity: selectedCategory ? 1 : 0.5 }}
                    disabled={!selectedCategory} {...selectFocusHandlers}
                  >
                    <option value="">— Sélectionner —</option>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ marginBottom: "16px" }}>
                ...
                <textarea
                  name="userMessage" value={formData.userMessage} onChange={handleChange}
                  placeholder="Ex: Mon PC est très lent depuis ce matin, impossible de travailler..."
                  rows={3} style={{ ...inputStyle, resize: "vertical", lineHeight: "1.5" }} {...focusHandlers}
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ex: Utilisation CPU à 100%, ventilateur tourne fort, 3 min pour ouvrir Chrome"
                  rows={2} style={{ ...inputStyle, resize: "vertical", lineHeight: "1.5" }} {...focusHandlers}
                />
              </div>
            </div>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ marginBottom: "16px" }}>
                ...
                <textarea
                  name="actionsTaken" value={formData.actionsTaken} onChange={handleChange}
                  placeholder={"Ex: 1) Ouvert Gestionnaire des tâches\n2) Identifié Windows Update à 95% CPU\n3) Redémarré le service wuauserv\n4) Redémarré le PC → problème résolu"}
                  rows={4} style={{ ...inputStyle, resize: "vertical", lineHeight: "1.5" }} {...focusHandlers}
                />
              </div>
              <div style={{ marginBottom: "16px" }}>
                <label style={labelStyle}>Cause racine</label>
                <input
                  name="rootCause" value={formData.rootCause} onChange={handleChange}
                  placeholder="Ex: Service Windows Update bloqué en boucle"
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <label style={{ ...labelStyle, margin: 0, display: "flex", alignItems: "center", gap: "8px", cursor: "pointer" }}>
                  <input
                    type="checkbox" name="resolved" checked={formData.resolved} onChange={handleChange}
                    style={{ width: "16px", height: "16px", accentColor: colors.success }}
                  />
                  <span style={{ textTransform: "none", fontSize: "14px", fontWeight: 400, color: colors.text }}>
                    Problème résolu
                  </span>
                </label>
              </div>
            ...
            <div style={cardStyle}>
              ...
              <textarea
                ...
                placeholder="Observations, contexte particulier, matériel concerné..."
                rows={2} style={{ ...inputStyle, resize: "vertical", lineHeight: "1.5" }} {...focusHandlers}
              />
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "center", paddingTop: "8px" }}>
              <button
                onClick={handleSubmit} style={btnPrimary}
                onMouseEnter={(e) => { e.target.style.transform = "translateY(-1px)"; e.target.style.boxShadow = `0 6px 28px ${colors.accentGlow}`; }}
                onMouseLeave={(e) => { e.target.style.transform = "translateY(0)"; e.target.style.boxShadow = `0 4px 20px ${colors.accentGlow}`; }}
              >
        ...
        {currentStep === "processing" && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "400px", gap: "24px" }}>
            ...
            <div style={{ textAlign: "center" }}>
              <p style={{ fontSize: "17px", fontWeight: 600, color: colors.white, margin: "0 0 8px" }}>
                Analyse IA en cours...
              </p>
              <p style={{ fontSize: "13px", color: colors.textMuted, margin: 0 }}>
                Extraction des métadonnées, mots-clés et structuration pour la base de connaissances
              </p>
            </div>
        ...
        {currentStep === "result" && kbEntry && (
          <div ref={resultRef} style={{ animation: "fadeIn 0.5s ease" }}>
            {/* Success banner */}
            <div style={{
              background: colors.successGlow, border: `1px solid ${colors.success}33`,
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <div>
                  <p style={{ margin: 0, fontWeight: 600, color: colors.success, fontSize: "15px" }}>
                    Entrée KB générée avec succès
                  </p>
              ...
              <button
                ...
                style={{
                  padding: "10px 20px", borderRadius: "8px", border: "none", cursor: saveStatus === "saved" ? "default" : "pointer",
                  fontFamily: font, fontSize: "13px", fontWeight: 600,
                  background: saveStatus === "saved" ? colors.successGlow : saveStatus === "error" ? "rgba(239,68,68,0.15)" : `linear-gradient(135deg, ${colors.success}, #059669)`,
                  color: saveStatus === "saved" ? colors.success : saveStatus === "error" ? colors.danger : colors.white,
                  border: saveStatus ? `1px solid ${saveStatus === "error" ? colors.danger : colors.success}44` : "none",
                  transition: "all 0.2s",
                }}
              >
            ...
            <div style={{ display: "flex", gap: "4px", marginBottom: "20px", background: colors.surfaceLight, borderRadius: "10px", padding: "4px" }}>
              {[
                { key: "visual", label: "Vue structurée" },
                { key: "json", label: "JSON (metadata.json)" },
                { key: "markdown", label: "Fiche Markdown" },
              ].map((tab) => (
                <button
                  key={tab.key}
                  ...
                  style={{
                    ...
                    color: activeTab === tab.key ? colors.white : colors.textDim,
              ...
              <div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "16px" }}>
                  <div style={{ ...cardStyle, borderLeft: `3px solid ${colors.danger}` }}>
                    ...
                    {kbEntry.symptoms && kbEntry.symptoms.length > 0 && (
                      <div style={{ marginTop: "12px" }}>
                        <div style={{ fontSize: "11px", color: colors.textMuted, marginBottom: "6px" }}>Symptômes :</div>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                          {kbEntry.symptoms.map((s, i) => (
                            <span key={i} style={{ padding: "3px 10px", borderRadius: "20px", fontSize: "11px", background: "rgba(239,68,68,0.1)", color: "#fca5a5", border: "1px solid rgba(239,68,68,0.2)" }}>{s}</span>
                          ))}
                        </div>
                      </div>
                    )}
                  ...
                  <div style={{ ...cardStyle, borderLeft: `3px solid ${colors.success}` }}>
                    ...
                    {kbEntry.solution_steps && kbEntry.solution_steps.length > 0 && (
                      <div>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                    {(kbEntry.keywords || []).map((kw, i) => (
                      <span key={i} style={{ padding: "4px 12px", borderRadius: "20px", fontSize: "12px", fontWeight: 500, background: colors.accentGlow, color: colors.accent, border: `1px solid ${colors.accent}33` }}>{kw}</span>
                    ))}
                  </div>
                  {kbEntry.related_keywords_en && kbEntry.related_keywords_en.length > 0 && (
                    <div style={{ marginTop: "10px", display: "flex", flexWrap: "wrap", gap: "6px" }}>
                      {kbEntry.related_keywords_en.map((kw, i) => (
                        <span key={i} style={{ padding: "4px 12px", borderRadius: "20px", fontSize: "11px", background: "rgba(148,163,184,0.1)", color: colors.textMuted, border: `1px solid ${colors.border}` }}>{kw}</span>
                      ))}
                    </div>
                  )}
                </div>

                {kbEntry.prevention_tips && kbEntry.prevention_tips.length > 0 && (
                  <div style={{ ...cardStyle, borderLeft: `3px solid ${colors.warning}` }}>
                    ...
                    {kbEntry.prevention_tips.map((tip, i) => (
                      <p key={i} style={{ margin: i < kbEntry.prevention_tips.length - 1 ? "0 0 6px" : 0, fontSize: "13px", color: colors.textDim }}>• {tip}</p>
                    ))}
                  </div>
                )}
              </div>
            ...
            {activeTab === "json" && (
              <div style={cardStyle}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                  <span style={{ fontSize: "12px", color: colors.textMuted }}>metadata.json — Prêt pour insertion dans PostgreSQL / ChromaDB</span>
                  <div style={{ display: "flex", gap: "8px" }}>
                    ...
                    <button onClick={() => handleDownload("json")} style={{ padding: "6px 14px", border: `1px solid ${colors.border}`, borderRadius: "6px", background: "transparent", color: colors.textDim, fontSize: "12px", cursor: "pointer", fontFamily: font, transition: "all 0.2s" }}>
                      ⬇ .json
                    </button>
                  </div>
                </div>
            ...
            {activeTab === "markdown" && (
              <div style={cardStyle}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                  <span style={{ fontSize: "12px", color: colors.textMuted }}>fiche-intervention.md — Format base de connaissances</span>
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button
                      onClick={() => setMdPreview((v) => !v)}
                      style={{ padding: "6px 14px", border: `1px solid ${mdPreview ? colors.accent : colors.border}`, borderRadius: "6px", background: mdPreview ? colors.accentGlow : "transparent", color: mdPreview ? colors.accent : colors.textDim, fontSize: "12px", cursor: "pointer", fontFamily: font, transition: "all 0.2s" }}
                    >
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: "12px", paddingTop: "16px", flexWrap: "wrap" }}>
              <button
                onClick={handleReset}
                style={{ ...btnPrimary, background: "transparent", color: colors.textDim, border: `1px solid ${colors.border}`, boxShadow: "none" }}
                onMouseEnter={(e) => { e.target.style.borderColor = colors.textDim; }}
                onMouseLeave={(e) => { e.target.style.borderColor = colors.border; }}
              >
                ← Nouvelle fiche
              </button>
              <button
                onClick={handleCopy} style={{ ...btnPrimary, background: colors.surfaceLight, boxShadow: "none", border: `1px solid ${colors.border}` }}
              ...
              <button
                onClick={() => handleDownload("json")} style={btnPrimary}
                onMouseEnter={(e) => { e.target.style.transform = "translateY(-1px)"; }}
                onMouseLeave={(e) => { e.target.style.transform = "translateY(0)"; }}
              >
                ⬇ Télécharger .json
              ...
              <button
                onClick={() => handleDownload("md")} style={{ ...btnPrimary, background: `linear-gradient(135deg, ${colors.success}, #059669)` }}
                onMouseEnter={(e) => { e.target.style.transform = "translateY(-1px)"; }}
                onMouseLeave={(e) => { e.target.style.transform = "translateY(0)"; }}
              >
                ⬇ Télécharger .md
...
// ── Generate Markdown output matching project template ──
